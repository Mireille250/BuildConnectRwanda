...
export class MessagingService {
  ...
  async getOrCreateConversation(userId: string, otherUserId: string) {
    ...
    const otherUser = await this.db.queryOne<{ id: string; first_name: string; last_name: string }>(
 ...
 async getMyConversations(userId: string) {
  const conversations = await this.db.queryMany(
    `SELECT
  ...
  return conversations.map((c) => ({
    ...
    unreadCount: parseInt(c.unread_count as string, 10),
  ...
  async saveMessage(
    ...
    const message = await this.db.queryOne(
      `INSERT INTO messages (conversation_id, sender_id, receiver_id, content, status)
       VALUES ($1, $2, $3, $4, 'SENT')
       RETURNING id, content, status, created_at, sender_id, receiver_id`,
